...
// Import variables prefixed with 'd9settings:' into $settings and 'd9config:'
// into $config.
if (getenv('PLATFORM_VARIABLES')) {
  $variables = json_decode(base64_decode(getenv('PLATFORM_VARIABLES')), TRUE);
  foreach ($variables as $name => $value) {
    // A variable named "d9settings:example-setting" will be saved in
    // $settings['example-setting'].
    if (strpos($name, 'd9settings:') === 0) {
    ...
    // A variable named "d9config:example-name:example-key" will be saved in
    // $config['example-name']['example-key'].
    elseif (strpos($name, 'd9config:') === 0 && substr_count($name, ':') >= 2) {
      list(, $config_key, $config_name) = explode(':', $name, 3);
      $config[$config_key][$config_name] = $value;
    }
    // A complex variable named "d9config:example-name" will be saved in
    // $config['example-name'].
    elseif (strpos($name, 'd9config:') === 0 && is_array($value)) {
